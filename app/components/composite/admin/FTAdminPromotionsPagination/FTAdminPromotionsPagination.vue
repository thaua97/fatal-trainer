...
const summaryLabel = computed(() => {
  if (props.total === 0) return 'Nenhuma promoção'
  if (props.total === 1) return '1 promoção'
  return `Mostrando ${rangeStart.value}–${rangeEnd.value} de ${props.total} promoções`
})
</script>
...
<template>
  <nav
    ...
    aria-label="Paginação de promoções"
    data-testid="admin-promotions-pagination"
  >
